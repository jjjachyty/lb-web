          ...
          <v-layout>
            <v-flex xs12 >
              <v-card>
                <v-layout row wrap align-center>
                  <v-flex xs12>
                    <v-text-field min="0" required prepend-icon="money" v-model.number="quotationOrder.charge" :rules="chargeRules" type="number" label="代购费(包邮):" clearable placeholder="包邮代购费"></v-text-field>
                  </v-flex>
                  <v-flex>
                    <v-dialog ref="timeDialog" v-model="timeDialog" :return-value.sync="quotationOrder.expiryTime" persistent lazy
                      full-width width="290px">

                      <v-text-field slot="activator" clearable required v-model="quotationOrder.expiryTime" :rules="expiryTimeRules" placeholder="报价截止时间" label="报价截止时间"  prepend-icon="access_time" readonly></v-text-field>
                      <v-time-picker v-model="quotationOrder.expiryTime" actions format="24hr" :min="minExpiryTime" locale="zh_CN">
                        ...
                        <v-btn flat color="primary" @click="$refs.timeDialog.save(quotationOrder.expiryTime)">确定</v-btn>
                                    ...
                                    <v-flex>
                    <v-dialog ref="deliveryTimeDialog" v-model="deliveryTimeDialog" :return-value.sync="quotationOrder.deliveryTime" persistent lazy
                      full-width width="290px">

                      <v-text-field slot="activator" v-model="quotationOrder.deliveryTime" placeholder="国内发货时间" required clearable :rules="deliveryTimeRules" label="国内发货时间"  prepend-icon="date" readonly></v-text-field>
